...
;layui.define(['index', 'admin', 'form', 'table', 'layer', 'laydate'], function (e) {
    ...
    var tableObject = {
        ...
        url: ctxPath + 'db-slowsql4mysql/get-slowsql-list?dbId=' + id,
    ...
    table.on('toolbar(list-table-slowsql)', function (obj) {
        ...
        if (obj.event === 'slowsqlBatchdel') {
            ...
            layer.confirm('确定删除吗？', function (index) {
                ...
                admin.req({
                    type: 'delete',
                    url: ctxPath + 'db-slowsql4mysql/delete-slowsql',
        ...
        if (obj.event === 'slowsqlCleanup') {
            layer.confirm('确定清空吗？', function (index) {
                admin.req({
                    type: 'delete',
                    url: ctxPath + 'db-slowsql4mysql/cleanup-slowsql',
                    dataType: 'json',
                    contentType: 'application/json;charset=utf-8',
                    headers: {
                        "X-CSRF-TOKEN": tokenValue
    ...
    table.on('tool(list-table-slowsql)', function (obj) {
        ...
        if (obj.event === 'slowsqlInfo') {
            ...
            var infoLayerIndex = layer.open({
                type: 2,
                title: '慢SQL详情',
                content: ctxPath + 'db-slowsql4mysql/slowsql-detail?id=' + data.id,
                maxmin: true,
                shade: 0,
                moveOut: true,
                area: [width, height],
