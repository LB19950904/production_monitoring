...
;layui.define(function (e) {
    layui.use(['index', 'admin', 'layer', 'fullscreen'], function () {
            ...
            //页面加载完，先第一次绘制拓扑图
            drawing(monitorHttpVos);

            // 获取HTTP接口拓扑图信息
            function getHttpTopologyInfo() {
                admin.req({
                    type: 'post',
                    url: layui.setter.base + 'monitor-topology/get-http-topology-info',
                    dataType: 'json',
            ...
            function drawing(data) {
                if (isNotEmpty(data) && data.length !== 0) {
                    ...
                    function wrapFromNodes() {
                        ...
                        // 服务端HTTP接口信息根据‘hostnameSource’去重
                        var formHttps = uniqueArr(data, 'hostnameSource');
                        formHttps.forEach((fromHttp, index) => {
                            // 中心点
                            var x = (stage.width / formHttps.length) * (index + 0.5), y = stage.height * 0.5;
                            // 主机名（来源）
                            var fromHostnameSource = fromHttp.hostnameSource;
                            // 添加根节点对象
                    ...
                    function wrapToNodes(fromNodes) {
                        ...
                        fromNodes.forEach((fromNode, fromHostnameSource) => {
                            ...
                            data.forEach((toHttp) => {
                                // 主机名（来源）
                                var toHostnameSource = toHttp.hostnameSource;
                                // URL地址（目的地）
                                var toUrlTarget = toHttp.urlTarget;
                                // 状态
                                var toStatus = toHttp.status;
                                // 匹配上：to节点属于此from节点
                                if (toHostnameSource === fromHostnameSource) {
                                    // 先拿，有就不再创建，没有才创建
                                    var toNode = toNodes.get(toUrlTarget);
                                    if (isEmpty(toNode)) {
                                        // 添加节点对象
                                        var point = randCirclePoint(x, y, 200);
                                        toNode = window.commonJtopo.addNode(layer, '', point[0], point[1], 128, 128, {
                                            imageSrc: (function () {
                                                if (toStatus === 200) {
                                        ...
                                        toNode.on('mouseenter', function (event) {
                                            // URL地址
                                            var urlTarget = this.userData.http.urlTarget;
                                            // 请求方法
                                            var method = this.userData.http.method;
                                            // 请求参数
                                            var parameter = this.userData.http.parameter;
                                            // 描述
                                            var descr = this.userData.http.descr;
                                            // 平均响应时间
                                            var avgTime = this.userData.http.avgTime;
                                            // 状态
                                            var thatStatus = this.userData.http.status;
                                            var status = (function () {
                                                if (thatStatus === 200) {
                                                    return '在线';
                                            ...
                                            // 离线次数
                                            var offlineCount = this.userData.http.offlineCount;
                                            // 监控环境
                                            var monitorEnv = this.userData.http.monitorEnv;
                                            // 监控分组
                                            var monitorGroup = this.userData.http.monitorGroup;
                                            // 异常信息
                                            var excMessage = this.userData.http.excMessage;
                                            var html = '<div class="layui-row layui-col-space10" style="word-wrap:break-word;">' +
                                                '<div class="layui-col-md12" style="float: none;"><label class="label-font-weight">URL：</label>' + urlTarget + '</div>' +
                                                '<div class="layui-col-md12" style="float: none;"><label class="label-font-weight">请求方法：</label>' + method + '</div>';
                                            if (isNotEmpty(parameter)) {
                                                html += '<div class="layui-col-md12" style="float: none;"><label class="label-font-weight">请求参数：</label>' + parameter + '</div>';
                                            }
                                            if (isNotEmpty(descr)) {
                                            ...
                                            if (isNotEmpty(excMessage)) {
                                                html += '<div class="layui-col-md12" style="float: none;"><label class="label-font-weight">异常信息：</label>' + excMessage + '</div>';
                                            }
                                            html += '</div>';
                                            // 鼠标指向小提示
                                        ...
                                        toNodes.set(toUrlTarget, toNode);
                                    }
                                }
                            });
                    ...
                    function wrapLinks(fromNodes, toNodes) {
                        ...
                        data.forEach(http => {
                            ...
                            var id = http.id;
                            // 主机名（来源）
                            var hostnameSource = http.hostnameSource;
                            // URL地址（目的地）
                            var urlTarget = http.urlTarget;
                            // 请求方法
                            var method = http.method;
                            // 状态
                            var status = http.status;
                            // 平均响应时间（毫秒）
                            var avgTime = isNotEmpty(http.avgTime) ? formatMillisecond(http.avgTime) : '';
                            // 源节点
                            var fromNode = fromNodes.get(hostnameSource);
                            // 目标节点
                            var toNode = toNodes.get(urlTarget);
                            if (isNotEmpty(fromNode) && isNotEmpty(toNode)) {
                                // 连线key
                                var myLinkKey = method + '-->' + urlTarget;
            ...
            function toAvgTimePage(id) {
                var avgTimeHistoryLayerIndex = l.open({
                    type: 2,
                    title: '访问耗时',
                    content: ctxPath + 'monitor-http/avg-time?id=' + id,
                    maxmin: true,
                    shade: 0,
                    moveOut: true,
