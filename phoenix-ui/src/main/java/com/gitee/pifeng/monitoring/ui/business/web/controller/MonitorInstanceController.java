...
public class MonitorInstanceController extends BaseController {
    ...
    public ModelAndView instanceDetail(Long id, String instanceId) {
        ...
        mv.addObject("endpoint", monitorInstanceVo.getEndpoint());
        // 是否启用Arthas
        boolean isEnableArthas = Boolean.TRUE.equals(monitorInstanceVo.getIsEnableArthas());
        // 是否收集VM指标
        boolean isCollectVmMetrics = Boolean.TRUE.equals(monitorInstanceVo.getIsCollectVmMetrics());
        // 是否收集线程池指标
        boolean isCollectThreadPoolMetrics = Boolean.TRUE.equals(monitorInstanceVo.getIsCollectThreadPoolMetrics());
        mv.addObject("isEnableArthas", isEnableArthas);
        mv.addObject("isCollectVmMetrics", isCollectVmMetrics);
        mv.addObject("isCollectThreadPoolMetrics", isCollectThreadPoolMetrics);
        if (isCollectVmMetrics) {
            // 获取jvm内存类型
        ...
        if (isCollectThreadPoolMetrics) {
            // 获取java线程池名字
            List<String> javaThreadPoolNames = this.monitorJavaThreadPoolService.getJavaThreadPoolNames(instanceId);
            mv.addObject("javaThreadPoolNames", javaThreadPoolNames);
            // 获取java线程池信息
            MonitorJavaThreadPoolVo monitorJavaThreadPoolVo = MonitorJavaThreadPoolVo.builder().build();
            if (CollectionUtils.isNotEmpty(javaThreadPoolNames)) {
                monitorJavaThreadPoolVo = this.monitorJavaThreadPoolService.getJavaThreadPoolInfo(instanceId, javaThreadPoolNames.get(0));
            }
            mv.addObject("monitorJavaThreadPoolVo", monitorJavaThreadPoolVo);
        }
        return mv;
    }
     ...
     * @param endpoint           应用端点
     * @param instanceId         应用实例ID
     * @param javaThreadPoolName Java线程池名称
     * @return {@link ModelAndView} Java线程池配置表单页面
     * @author 皮锋
     * @custom.date 2026/3/19 17:24
     */
    @Operation(summary = "访问Java线程池配置表单页面")
    @Parameters(value = {
            @Parameter(name = "endpoint", description = "应用端点", required = true, in = ParameterIn.QUERY),
            @Parameter(name = "instanceId", description = "应用实例ID", required = true, in = ParameterIn.QUERY),
            @Parameter(name = "javaThreadPoolName", description = "Java线程池名称", in = ParameterIn.QUERY)})
    @GetMapping("/set-instance-java-thread-pool-form")
    public ModelAndView setInstanceJavaThreadPoolForm(String endpoint, String instanceId, String javaThreadPoolName) {
        ModelAndView mv = new ModelAndView("instance/setup-instance-java-thread-pool");
        mv.addObject("endpoint", endpoint);
        // 获取java线程池信息
        MonitorJavaThreadPoolVo javaThreadPoolInfo = this.monitorJavaThreadPoolService.getJavaThreadPoolInfo(instanceId, javaThreadPoolName);
        mv.addObject("javaThreadPoolInfo", javaThreadPoolInfo);
        return mv;
    }
     ...
     * @author 皮锋
     * @custom.date 2026/3/21 23:23
     */
    @Operation(summary = "配置Java线程池")
    @PutMapping("/set-instance-java-thread-pool")
    @PreAuthorize("hasAuthority('超级管理员')")
    @ResponseBody
    @OperateLog(operModule = UiModuleConstants.INSTANCE, operType = OperateTypeConstants.UPDATE, operDesc = "配置Java线程池")
    public LayUiAdminResultVo setInstanceJavaThreadPool(MonitorJavaThreadPoolVo monitorJavaThreadPoolVo) throws IOException {
        return this.monitorJavaThreadPoolService.setInstanceJavaThreadPool(monitorJavaThreadPoolVo);
    }
