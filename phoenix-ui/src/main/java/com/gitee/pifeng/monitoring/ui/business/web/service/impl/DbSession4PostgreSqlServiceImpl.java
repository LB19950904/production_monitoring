...
import com.gitee.pifeng.monitoring.ui.business.web.service.IDbSession4PostgreSqlService;
import com.gitee.pifeng.monitoring.ui.business.web.vo.DbSession4PostgreSqlVo;
...
@Slf4j
@Service
public class DbSession4PostgreSqlServiceImpl implements IDbSession4PostgreSqlService {
     ...
     * @author 皮锋
     * @custom.date 2025/6/13 17:00
     */
    @Override
    public Page<DbSession4PostgreSqlVo> getSessionList(Long current, Long size, Long id, String userParam, String hostParam,
        ...
        // 从服务端获取数据
        String resultStr = Sender.send(UrlConstants.POSTGRESQL_GET_SESSION_LIST_URL, baseRequestPackage.toJsonString());
        ...
        // 解析数据
        List<DbSession4PostgreSqlVo> dbSession4PostgreSqlVos = Lists.newArrayList();
        for (Entity entity : entityList) {
            ...
            DbSession4PostgreSqlVo dbSession4PostgreSqlVo = DbSession4PostgreSqlVo.builder()
                    .id(sessionId)
                    .user(user)
                    .host(host)
                    .db(db)
                    ...
                    .info(info)
                    .build();
            dbSession4PostgreSqlVos.add(dbSession4PostgreSqlVo);
        }
        if (StringUtils.isNotBlank(userParam)) {
            dbSession4PostgreSqlVos = dbSession4PostgreSqlVos.stream()
        ...
        // 设置返回对象
        Page<DbSession4PostgreSqlVo> dbSession4PostgreSqlVoPage = new Page<>();
        dbSession4PostgreSqlVoPage.setRecords(dbSession4PostgreSqlVos);
        dbSession4PostgreSqlVoPage.setTotal(dbSession4PostgreSqlVos.size());
        dbSession4PostgreSqlVoPage.setCurrent(current);
        dbSession4PostgreSqlVoPage.setSize(size);
        return dbSession4PostgreSqlVoPage;
    }
     ...
     * </p>
     *
     * @param sessionIds PostgreSQL数据库会话ID集合
     * @param id         数据库ID
    ...
    public LayUiAdminResultVo destroySession(List<Long> sessionIds, Long id) throws NetException, IOException {
        ...
        BaseRequestPackage baseRequestPackage = this.uiPackageConstructor.structureBaseRequestPackage(extraMsg);
        // 从服务端获取数据
        String resultStr = Sender.send(UrlConstants.POSTGRESQL_DESTROY_SESSION_URL, baseRequestPackage.toJsonString());
        BaseResponsePackage baseResponsePackage = JSON.parseObject(resultStr, BaseResponsePackage.class);
        Result result = baseResponsePackage.getResult();
