...
public class MonitorDbServiceImpl extends ServiceImpl<IMonitorDbDao, MonitorDb> implements IMonitorDbService {
     ...
     */
    @Autowired
    private IMonitorDbSlowSqlMysqlDao monitorDbSlowSqlMysqlDao;

    /**
     * Oracle数据库慢SQL表数据访问对象
     */
    @Autowired
    private IMonitorDbSlowSqlOracleDao monitorDbSlowSqlOracleDao;
    ...
    public LayUiAdminResultVo deleteMonitorDb(List<Long> ids) {
        // 删除MySQL数据库慢SQL数据
        LambdaQueryWrapper<MonitorDbSlowSqlMysql> mysqlLambdaQueryWrapper = new LambdaQueryWrapper<>();
        mysqlLambdaQueryWrapper.in(MonitorDbSlowSqlMysql::getDbId, ids);
        this.monitorDbSlowSqlMysqlDao.delete(mysqlLambdaQueryWrapper);
        // 删除Oracle数据库慢SQL数据
        LambdaQueryWrapper<MonitorDbSlowSqlOracle> oracleLambdaQueryWrapper = new LambdaQueryWrapper<>();
        oracleLambdaQueryWrapper.in(MonitorDbSlowSqlOracle::getDbId, ids);
        this.monitorDbSlowSqlOracleDao.delete(oracleLambdaQueryWrapper);
        // 删除数据库表数据
