    ...
    public MonitorAlarmRecordVo getMonitorAlarmRecord(Long id) {
        ...
        monitorAlarmRecordVo.setType(StringUtils.replaceEach(type,
                new String[]{MonitorTypeEnums.SERVER.name(), MonitorTypeEnums.NET.name(),
                        ...
                        MonitorTypeEnums.DOCKER.name(), MonitorTypeEnums.INSTANCE.name(),
                        MonitorTypeEnums.CUSTOM.name(), MonitorTypeEnums.DATABASE.name(),
                        MonitorTypeEnums.NETWORK_DEVICE.name()},
                new String[]{"服务器", "网络", "TCP服务", "HTTP服务", "Docker", "应用程序", "自定义", "数据库", "网络设备"}));
        ...
        monitorAlarmRecordVo.setWay(StringUtils.isBlank(way) ? way : way.replace(",", "、")
                ...
                .replace(AlarmWayEnums.DINGTALK.name(), "钉钉")
                .replace(AlarmWayEnums.EPWECHAT.name(), "企业微信")
                .replace(AlarmWayEnums.FEISHU.name(), "飞书"));
        // 告警内容
