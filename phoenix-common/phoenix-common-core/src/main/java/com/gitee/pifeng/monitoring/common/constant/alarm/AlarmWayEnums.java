...
public enum AlarmWayEnums {
    ...
    public static AlarmWayEnums str2Enum(String alarmWayStr) {
        ...
        // 钉钉
        if (StringUtils.equalsIgnoreCase(AlarmWayEnums.DINGTALK.name(), alarmWayStr)) {
            return AlarmWayEnums.DINGTALK;
        }
        // 企业微信
        if (StringUtils.equalsIgnoreCase(AlarmWayEnums.EPWECHAT.name(), alarmWayStr)) {
            return AlarmWayEnums.EPWECHAT;
        }
        // 飞书
        if (StringUtils.equalsIgnoreCase(AlarmWayEnums.FEISHU.name(), alarmWayStr)) {
            return AlarmWayEnums.FEISHU;
        }
        return null;
    }
