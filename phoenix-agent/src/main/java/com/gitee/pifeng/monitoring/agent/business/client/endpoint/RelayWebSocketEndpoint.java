package com.gitee.pifeng.monitoring.agent.business.client.endpoint;

import com.gitee.pifeng.monitoring.agent.business.server.endpoint.UpstreamClientEndpoint;
...
public class RelayWebSocketEndpoint {
    ...
    public void onMessage(String message, Session downstreamSession) {
        ...
        String subPath = (String) downstreamSession.getUserProperties().get("subPath");
        // 监控业务
        ...
        upstreamSession.getAsyncRemote().sendText(message, sendResult -> {
            if (sendResult.isOK()) {
                if (log.isDebugEnabled()) {
                    log.debug("WebSocket消息转发成功，下游Session[{}] -> 上游Session[{}]，消息长度：{}", downstreamSessionId, upstreamSessionId, finalMessage.length());
                }
            } else {
