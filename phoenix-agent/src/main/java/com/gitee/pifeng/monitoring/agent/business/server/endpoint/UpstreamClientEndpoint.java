package com.gitee.pifeng.monitoring.agent.business.server.endpoint;

import com.gitee.pifeng.monitoring.agent.business.client.endpoint.RelayWebSocketEndpoint;
...
public class UpstreamClientEndpoint {
    ...
    public void onMessage(String message, Session upstreamSession) {
        ...
        String subPath = (String) upstreamSession.getUserProperties().get("subPath");
        // 监控业务
        if (WebSocketBusinessTypeConstants.MONITORING.equals(subPath)) {
            ...
            AbstractSuperPackage superPackage = (AbstractSuperPackage) payload;
            // 添加链路信息
        ...
        downstreamSession.getAsyncRemote().sendText(message, sendResult -> {
            if (sendResult.isOK()) {
                if (log.isDebugEnabled()) {
                    log.debug("WebSocket消息转发成功，上游Session[{}] -> 下游Session[{}]，消息长度：{}，消息：{}", upstreamSessionId, downstreamSessionId, finalMessage.length(), finalMessage);
                }
            } else {
