...
@Component
@Slf4j
public class ServerLoadAverageMonitor implements IServerListener {
    ...
    private void sendAlarmInfo(String title, MonitorServer monitorServer, Integer logicalProcessorCount,
        ...
        msgBuilder.append("IP地址/IP：").append(ip).append("，<br>服务器/Server：").append(serverName);
        if (StringUtils.isNotBlank(serverSummary)) {
            msgBuilder.append("，<br>描述/Desc：").append(serverSummary);
        }
        if (StringUtils.isNotBlank(monitorEnv)) {
            msgBuilder.append("，<br>环境/Env：").append(monitorEnv);
        }
        if (StringUtils.isNotBlank(monitorGroup)) {
            msgBuilder.append("，<br>分组/Group：").append(monitorGroup);
        }
        msgBuilder.append("，<br>CPU逻辑核数/CPU cores：").append(logicalProcessorCount)
                .append("，<br>1分钟平均负载/Average load over 1minute：").append(loadAverage1minutes)
                .append("，<br>5分钟平均负载/Average load over 5minute：").append(loadAverage5minutes)
                .append("，<br>15分钟平均负载/Average load over 15minute：").append(loadAverage15minutes);
        msgBuilder.append("，<br>平均负载参考值/Average load reference value：");
        ...
        msgBuilder.append("，<br>时间/Time：").append(DateTimeUtils.dateToString(new Date()));
