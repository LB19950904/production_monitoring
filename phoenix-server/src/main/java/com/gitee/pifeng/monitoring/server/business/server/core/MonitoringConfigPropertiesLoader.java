...
public class MonitoringConfigPropertiesLoader {
    ...
    private synchronized void setMonitoringProperties(MonitoringProperties properties) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("设置到监控配置属性对象成功，属性详情：{}", monitoringProperties.toJsonString());
        }
    }
    ...
    private MonitoringProperties setDefaultMonitorConfig() {
        ...
        // 告警钉钉配置属性
        MonitoringAlarmDingtalkProperties alarmDingtalkProperties = new MonitoringAlarmDingtalkProperties(null, null, true, null);
        // 企业微信配置属性
        MonitoringAlarmEnterpriseWechatProperties alarmEnterpriseWechatProperties = new MonitoringAlarmEnterpriseWechatProperties(null, true, null);
        // 告警飞书配置属性
        MonitoringAlarmFeishuProperties alarmFeishuProperties = new MonitoringAlarmFeishuProperties(null, null, true, null);
        // 告警配置属性
        MonitoringAlarmProperties alarmProperties = new MonitoringAlarmProperties(false,
                ...
                1,
                1,
                new AlarmWayEnums[]{AlarmWayEnums.MAIL, AlarmWayEnums.SMS, AlarmWayEnums.DINGTALK, AlarmWayEnums.EPWECHAT, AlarmWayEnums.FEISHU},
                alarmSmsProperties,
                alarmMailProperties,
                alarmDingtalkProperties,
                alarmEnterpriseWechatProperties,
                alarmFeishuProperties);
        // 网络状态配置属性
        ...
        // 应用实例线程池配置属性
        MonitoringInstanceThreadPoolProperties instanceThreadPoolProperties = new MonitoringInstanceThreadPoolProperties(true, true);
        // 应用实例配置属性
        MonitoringInstanceProperties instanceProperties = new MonitoringInstanceProperties(true, instanceStatusProperties, instanceThreadPoolProperties);
        // 服务器状态配置属性
        ...
        // 数据库慢SQL配置属性
        MonitoringDbSlowSqlProperties dbSlowSqlProperties = new MonitoringDbSlowSqlProperties(true, true, 60L, AlarmLevelEnums.INFO);
        // 数据库配置
        MonitoringDbProperties dbProperties = new MonitoringDbProperties(true, dbStatusProperties, dbTableSpaceProperties, dbSlowSqlProperties);
        // docker服务状态配置属性
        MonitoringDockerStatusProperties dockerStatusProperties = new MonitoringDockerStatusProperties(true, true);
        // docker配置属性
        MonitoringDockerProperties dockerProperties = new MonitoringDockerProperties(true, dockerStatusProperties);
        // 网络设备状态配置属性
        MonitoringNetworkDeviceStatusProperties networkDeviceStatusProperties = new MonitoringNetworkDeviceStatusProperties(true, true);
        // 网络设备配置属性
        MonitoringNetworkDeviceProperties networkDeviceProperties = new MonitoringNetworkDeviceProperties(true, networkDeviceStatusProperties);
        // 监控配置属性
        MonitoringProperties properties = new MonitoringProperties(5, alarmProperties, networkProperties, tcpProperties, httpProperties, instanceProperties, serverProperties, dbProperties, dockerProperties, networkDeviceProperties);
        // 查询数据库中是否有配置记录
        MonitorConfig monitorConfig = this.configService.getOne(new LambdaQueryWrapper<>());
        if (monitorConfig == null) {
            // 插入记录
