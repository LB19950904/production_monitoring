...
public class DbTableSpaceMonitorJob extends QuartzJobBean {
    ...
    private void sendAlarmInfo(String title, MonitorDb monitorDb, DbTableSpace dbTableSpace, AlarmLevelEnums alarmLevelEnum, AlarmReasonEnums alarmReasonEnum) throws NetException {
        ...
        builder.append("，<br>表空间名/Tablespace name：").append(dbTableSpace.getTablespaceName());
        builder.append("，<br>总空间/Total space：").append(dbTableSpace.getTotal());
        builder.append("，<br>使用空间/Used space：").append(dbTableSpace.getUsed());
        builder.append("，<br>剩余空间/Free space：").append(dbTableSpace.getFree());
        builder.append("，<br>使用率/Usage rate：").append(dbTableSpace.getUsedRate()).append("%");
        builder.append("，<br>剩余率/Surplus rate：").append(dbTableSpace.getFreeRate()).append("%");
