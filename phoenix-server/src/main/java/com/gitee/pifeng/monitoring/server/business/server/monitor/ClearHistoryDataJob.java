...
public class ClearHistoryDataJob extends QuartzJobBean {
    ...
    protected void executeInternal(@NonNull JobExecutionContext jobExecutionContext) {
        try {
            ...
            int clearJavaThreadPoolHistoryDataNum = this.javaThreadPoolService.clearHistoryData(historyTime);
            log.info("清理JAVA线程池历史数据：{} 条！", clearJavaThreadPoolHistoryDataNum);
            ...
            int clearDockerHistoryDataNum = this.dockerService.clearHistoryData(historyTime);
            log.info("清理DOCKER历史数据：{} 条！", clearDockerHistoryDataNum);
        } catch (Exception e) {
