...
public class AlarmRecordServiceImpl extends ServiceImpl<IMonitorAlarmRecordDao, MonitorAlarmRecord> implements IAlarmRecordService {
    ...
    public Long insertMonitorAlarmRecordToDb(Alarm alarm, String alarmCode) {
        ...
        // 告警内容标题(英文)
        String alarmTitleEn = alarm.getTitleEn();
        // 如果Alarm对象中没有英文标题，则根据中文标题查找对应的英文标题
        if (StringUtils.isBlank(alarmTitleEn) && StringUtils.isNotBlank(alarmTitle)) {
            alarmTitleEn = MonitorEventTitleEnum.getEnglishTitle(alarmTitle);
        }
        // 告警内容
        ...
        monitorAlarmRecord.setTitleEn(alarmTitleEn);
