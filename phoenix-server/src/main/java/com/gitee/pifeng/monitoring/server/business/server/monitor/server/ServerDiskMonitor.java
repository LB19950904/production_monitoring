...
@Component
@Slf4j
public class ServerDiskMonitor implements IServerListener {
    ...
    private void sendAlarmInfo(String title, MonitorServer monitorServer, String devName, String dirName, String total,
        ...
        msgBuilder.append("，<br>磁盘分区名称/Disk partition name：").append(devName)
                .append("，<br>磁盘分区路径/Disk partition path：").append(dirName)
                .append("，<br>磁盘分区使用量/Disk partition usage：").append(used).append(" / ").append(total)
                .append("，<br>磁盘分区使用率/Disk partition usage rate：").append(usePercent).append("%");
