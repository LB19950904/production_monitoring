...
public class HttpMonitorJob extends QuartzJobBean {
    ...
    private MonitoredThreadPoolExecutor httpMonitorThreadPoolExecutor;
    ...
    private void sendAlarmInfo(String title, AlarmLevelEnums alarmLevelEnum, AlarmReasonEnums alarmReasonEnum, MonitorHttp monitorHttp) {
        ...
        // URL地址（目的地）
        String urlTarget = monitorHttp.getUrlTarget();
        builder.append("源主机/Source host：").append(hostnameSource)
                .append("，<br>目标URL/Target url：").append(urlTarget)
                .append("，<br>请求方法/Request method：").append(method)
                .append("，<br>请求头/Request header：").append(headerParameter);
        // POST请求
        if (StringUtils.equalsIgnoreCase(HttpMethod.POST.name(), method)) {
            builder.append("，<br>媒体类型/Media type：").append(contentType);
            if (StringUtils.isNotBlank(bodyParameter)) {
                JSONObject bodyParameterJson = JSON.parseObject(bodyParameter);
                if (StringUtils.equalsIgnoreCase(contentType, MediaType.APPLICATION_FORM_URLENCODED_VALUE)) {
                    ...
                    builder.append("，<br>请求体/Request body：").append(bodyParameter);
                    ...
                    builder.append("，<br>请求体/Request body：").append(bodyJsonParameter);
                }
            }
        }
        if (StringUtils.isNotBlank(monitorHttp.getDescr())) {
            builder.append("，<br>描述/Desc：").append(monitorHttp.getDescr());
        }
        if (StringUtils.isNotBlank(monitorHttp.getMonitorEnv())) {
            builder.append("，<br>环境/Env：").append(monitorHttp.getMonitorEnv());
        }
        if (StringUtils.isNotBlank(monitorHttp.getMonitorGroup())) {
            builder.append("，<br>分组/Group：").append(monitorHttp.getMonitorGroup());
        }
